...
// نداء يطلب ردًا بصيغة JSON، مع إعادة محاولة تلقائية تميّز بين نوعين من
// الفشل: فشل شبكة (انقطاع/مهلة) يحتاج انتظار أطول لأن سبب الانقطاع (تبديل
// شبكة، تجميد التطبيق بالخلفية) قد يزول خلال ثوانٍ، فنعطيه محاولات أكثر
// وفواصل أطول؛ وفشل صيغة الرد (JSON غير صالح/مبتور) يحتاج فقط حد رموز أعلى
// فور المحاولة التالية، فمهلته أقصر.
    ...
    for (let i = 0; i < attempts; i++) {
        try {
            ...
            if (i < attempts - 1) {
                ...
                // فشل شبكة: انتظار أطول ومتصاعد (3، 6، 9 ثوانٍ) يعطي فرصة
                // حقيقية للشبكة ترجع. فشل صيغة رد: انتظار قصير ثابت (1.2 ثانية)
                const delay = isNetworkErr ? 3000 * (i + 1) : 1200;
