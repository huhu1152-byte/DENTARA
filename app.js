...
async function callAIJSON(prompt, options = {}) {
    // ملاحظة: خفّضنا العدد الافتراضي من 3 لـ 2 — مع مهلة 110 ثانية للمحاولة
    // الواحدة، 3 محاولات كانت تعني حتى 5+ دقائق انتظار قبل ما يظهر أي خطأ
    // للطبيب حتى لو فشلت كلها. محاولتين تكفي لتغطية أغلب الإخفاقات العابرة
    // (شبكة، رد JSON غير صالح) بدون إطالة الانتظار الأسوأ بشكل غير معقول.
    ...
    for (let i = 0; i < attempts; i++) {
        try {
            const raw = await callAI(prompt, {
                ...options,
                // نرفع الحد بشكل أكبر في كل محاولة (800 بدل 400) — لو صار
                // ترميز (truncation) بالمحاولة الأولى، لازم قفزة حقيقية
                // بحد الرموز تحل المشكلة فعليًا، مو زيادة رمزية صغيرة تتكرر
                // بنفس النتيجة الناقصة
