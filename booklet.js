...
async function bookletTasks() {
    ...
    const list = (r.personalized_instructions || []).slice(0, 4).map(i => ({
        title: i.instruction_arabic || "",
        times: i.when || ""
    }));
...
async function buildBooklet() {
    ...
    const rec = (App.current && App.current.record) || {};
    const r = (App.current && App.current.result) || {};
    const goals = (r.smart_goals || []).map(g => g.goal_arabic).filter(Boolean);
    ...
    <div class="bk-page">
        <div class="bk-panel bk-instructions-panel">
            ...
            ${(r.personalized_instructions || []).length ? `
            <div class="bk-ai-list">
                ${(r.personalized_instructions || []).map((ins, i) => `
                    <div class="bk-ai-item">
                        <span class="bk-how-num">${i + 1}</span>
                        <div>
                            <b>${escapeHtml(ins.instruction_arabic || "")}</b>
                            ${ins.how_to_do_it_arabic ? `<p>${escapeHtml(ins.how_to_do_it_arabic)}</p>` : ""}
                            ${ins.when ? `<span class="bk-ai-times"><i class="fa-solid fa-clock"></i> ${escapeHtml(ins.when)}</span>` : ""}
                        </div>
                    </div>`).join("")}
            </div>` : `<div class="bk-goalbox bk-tall"></div>`}
        </div>
