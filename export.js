...
async function exportAllToWord() {
    ...
    try {
        ...
        sorted.forEach((rec, idx) => {
            ...
            try { ins = JSON.parse(rec.personalizedInstructions || "[]"); } catch (e) {}
            try { barr = JSON.parse(rec.barriersSolutions || "[]"); } catch (e) {}
            try { answers = JSON.parse(rec.answers || "{}"); } catch (e) {}

            let probs = [];
            try { probs = (JSON.parse(rec.priorityProblems || "[]")).map(p => p.problem).filter(Boolean); } catch (e) {}
            let goals = [];
            ...
            const vids = recommendedVideos
                .map(rv => (App.videos || []).find(v => String(v.id) === String(rv.video_id)))
            ...
            <div class="rec">
                ...
                ${ins.length ? `<h3>${esc(L.ins)}</h3><ol>${ins.map(i =>
                    `<li><b>${esc(i.instruction_arabic || "")}</b> — ${esc(i.how_to_do_it_arabic || "")}${
                        i.when ? ` <i>(${esc(i.when)})</i>` : ""}</li>`).join("")}</ol>` : ""}

                ${goals.length ? `<h3>${esc(L.goals)}</h3><ul>${goals.map(x => `<li>${esc(x)}</li>`).join("")}</ul>` : ""}

                ${barr.length ? `<h3>${esc(L.barr)}</h3><ul>${barr.map(b =>
                    `<li><b>${esc(b.barrier || "")}</b> → ${esc(b.solution_arabic || "")}</li>`).join("")}</ul>` : ""}

                ${rec.motivationalMessage ? `<h3>${esc(L.motiv)}</h3><p>${esc(rec.motivationalMessage)}</p>` : ""}
