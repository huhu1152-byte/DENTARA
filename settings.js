            ...
            : App.videos.map(v => `
                <div class="q-item">
                    <div class="q-item-body">
                        ...
                        ${v.key ? `<span style="display:inline-block;background:#e3f2fd;color:#1565c0;border-radius:6px;padding:2px 8px;font-size:12px;margin-inline-start:6px;">${t((STANDARD_VIDEO_KEYS.find(k => k.key === v.key) || {}).labelKey || "")}</span>` : ""}
                        ${v.description ? `<span>${escapeHtml(v.description)}</span>` : (v.key ? "" : `<span style="color:#c62828;">${t("noDesc")}</span>`)}
...
// ======== الفيديوهات الخمسة الثابتة: تعرّف تلقائي من العنوان، وإذا لم
// يتضح منه يُفحص الوصف بنفس الكلمات المفتاحية ========
// بدل ما تختار الطبيبة تصنيفًا يدويًا، النظام يقرأ عنوان الفيديو أولًا،
// وإن لم يجد تطابقًا واضحًا يبحث بنص الوصف كذلك. الترتيب مهم: "Interdental
// Brush" لازم تُفحص قبل "brush" العامة حتى لا تُصنَّف غلط كفيديو تفريش.
function matchVideoKeyFrom(text) {
    const t = String(text || "").toLowerCase();
    if (/interdental|بين\s*الأسنان|بين\s*الاسنان|فرشاة\s*ما\s*بين/.test(t)) return "interdental_brush";
    if (/floss|خيط\s*الأسنان|خيط\s*الاسنان/.test(t)) return "dental_floss";
    if (/mouthwash|mouth\s*rinse|oral\s*rinse|غسول/.test(t)) return "mouthwash";
    if (/tongue|scraper|لسان|كاشط/.test(t)) return "tongue_scraper";
    if (/tooth\s*brush|brushing/.test(t) || (/تفريش|تنظيف/.test(t) && !/بين/.test(t))) return "toothbrushing";
    return "";
}
function guessVideoKey(title, description) {
    return matchVideoKeyFrom(title) || matchVideoKeyFrom(description);
}

const STANDARD_VIDEO_KEYS = [
    { key: "toothbrushing",    labelKey: "videoCategoryToothbrushing" },
    { key: "dental_floss",     labelKey: "videoCategoryFloss" },
    { key: "mouthwash",        labelKey: "videoCategoryMouthwash" },
    { key: "tongue_scraper",   labelKey: "videoCategoryTongueScraper" },
    { key: "interdental_brush",labelKey: "videoCategoryInterdental" }
];
...
function openVideoForm(id = null) {
    ...
    openModal(id ? t("editVideo") : t("addVideo"), `
        <div class="form-group"><label>${t("videoTitle")}</label>
            <input id="vf_title" value="${escapeHtml(v.title || "")}" placeholder="e.g. Tooth Brushing Technique">
            <small>${t("videoCategoryHint")}</small></div>
    ...
    `, async () => {
        ...
        const key = guessVideoKey(title, $("vf_desc").value.trim());
        const data = { title, url: normalizeVideoUrl(url), description: $("vf_desc").value.trim(), key };
        if (id) {
...
// ======== تحويل روابط الرفع إلى روابط مشاهدة مباشرة (قابلة للتضمين) ========
// الطبيب يرفع الفيديو على Google Drive أو يوتيوب وينسخ رابط المشاركة؛ هذا
// الرابط لا يُشغّل الفيديو مباشرة بمشغّل مدمج، فنحوّله تلقائيًا لصيغة تضمين
// (iframe) تشتغل بنفس صفحة المريض مباشرة بلا فتح تطبيق أو تبويب خارجي
// ولا تسجيل دخول لأي حساب
function normalizeVideoUrl(url) {
    ...
    // يوتيوب: watch?v=، youtu.be/، shorts/  →  embed/<ID>
    const yt = u.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/shorts\/|youtube\.com\/embed\/)([\w-]{6,})/);
    if (yt) return `https://www.youtube.com/embed/${yt[1]}`;

    return u;
}

// هل هذا الرابط قابل للتضمين المباشر (iframe) بنفس الصفحة؟
function isEmbeddableVideoUrl(url) {
    const u = String(url || "");
    return /drive\.google\.com\/file\/d\/.+\/preview/.test(u) || /youtube\.com\/embed\//.test(u);
}

// رابط ملف فيديو مباشر (مثلاً مرفوع على GitHub نفسه) يُشغَّل بمشغّل <video>
// عادي بدل iframe — يُعرف من امتداد الملف بنهاية الرابط
