...
const DENTARA_METHODOLOGY = [
    ...
    "",
    "SELF-CHECK before answering — did I connect the data instead of just repeating fields? Is every priority problem, instruction, aid, and video choice actually justified by this specific patient's real answers, with nothing invented? Is the patient-facing Arabic text simple, warm, and non-judgmental, easy for a regular Iraqi adult patient to understand? Is the dentist-facing text (dentist_summary, ai_explainability) concise, clinical, and traceable, in English?"
].join("\n");
...
// ======== بناء قائمة الفيديوهات المرقّمة للنموذج (بمعرّف id الحقيقي) ========
function buildVideoList() {
    return (App.videos || []).filter(v => v.id).map(v =>
        `- video_id: ${v.id} | العنوان: ${v.title || "بلا عنوان"}${v.description ? " | الوصف: " + v.description : ""}`
    ).join("\n");
}

// يتحقق أن كل video_id أرجعه النموذج موجود فعليًا بقائمة الفيديوهات —
// يمنع أي معرّف مُختلَق (المبدأ: "Never invent video_id" بالمنهجية أعلاه)
function knownVideoIds() {
    return new Set((App.videos || []).map(v => v.id).filter(Boolean));
}
...
async function analyzeRecord(record, answers) {
  ...
  "video_evaluation": [
    {"video_id": "exact video_id copied from the list above", "relevant": true, "reason": "short reason tied to this patient's answers", "related_problem": "which priority_problems.problem this connects to, or empty string if relevant:false"}
  ],
  "recommended_videos": [
    {"priority": 1, "video_id": "exact video_id copied from the list above", "title": "exact title copied from the list above", "reason": "why this patient specifically needs it", "related_problem": "which priority_problems.problem this addresses"}
  ],
  "ai_explainability": [
    {"recommendation": "short name of the recommendation being explained", "patient_data": ["exact answer(s) used"], "finding": "short clinical/behavioral finding", "reason": "why this matters for this patient", "expected_behavioral_or_clinical_benefit": "short expected benefit", "video_id": "matching video_id if any, else empty string"}
  ],
  ...
  "missing_information": ["short description of any information that was genuinely missing/blank and limited this analysis"]
        ...
        "",
        "video_evaluation MUST include exactly one entry for EVERY video_id in the numbered list above, in the same order — evaluate each one individually against this patient's top-3 priority problems before deciding. Do not skip any video_id, and do not stop early once you find 1-2 relevant ones. There is no minimum or maximum count — mark relevant:true for as many or as few as this patient's real answers justify, including all of them or none of them.",
        "related_problem (in video_evaluation, recommended_videos and ai_explainability) must name one of this patient's actual priority_problems.problem values — never a generic label. Leave it an empty string when not applicable.",
        "recommended_videos must be derived from video_evaluation: only video_ids marked relevant:true belong in recommended_videos, each carrying its exact video_id and title copied verbatim from the list above (never invented). If none are relevant, recommended_videos is an empty array. All recommended videos go in this ONE list — there is no weekly video assignment.",
        "",
        "priority_problems: list only the top 3, each with real evidence quoted/paraphrased from the actual answers.",
        ...
        "ai_explainability: one entry per major recommendation (each priority problem and/or each recommended video), following PATIENT DATA → FINDING → REASON → RECOMMENDATION → (MATCHING VIDEO if any). Never fabricate a reason.",
        ...
        LANG === "ar"
            ? "IMPORTANT: write every patient-facing text value (patient_summary.arabic, personalized_instructions, oral_hygiene_aids, smart_goals, barriers_and_solutions.solution_arabic, motivational_message_arabic) in ARABIC (simple, clear Arabic the patient can understand). Keep dentist_summary and ai_explainability in English as specified."
            : "IMPORTANT: patient-facing Arabic fields must still be written in Arabic regardless of the interface language, since the patient reads Arabic. Keep dentist_summary and ai_explainability in English as specified.",
        AI_SAFETY
    ].join("\n");

    const result = await callAIJSON(prompt, { maxTokens: 4200, temperature: 0.4 });

    // تحقق من صحة كل video_id قبل الحفظ — أي معرّف مُختلَق يُستبعد فورًا
    // (مبدأ "Never invent video_id" بالمنهجية أعلاه)
    const validIds = knownVideoIds();
    const recommendedVideos = (result.recommended_videos || [])
        .filter(v => v && validIds.has(v.video_id))
        .map(v => ({
            priority: v.priority || 0,
            video_id: v.video_id,
            title: v.title || (App.videos.find(x => x.id === v.video_id) || {}).title || "",
            reason: v.reason || "",
    ...
    const update = {
        ...
        aiExplainability: JSON.stringify(result.ai_explainability || []),
