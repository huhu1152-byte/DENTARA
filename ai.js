...
// ملاحظة مهمة (كلفة): فصلنا البروبمت لجزئين — `system` (ثابت 100%، نفسه
// حرفيًا بكل تحليل لأي مريض) و`prompt` (فقط بيانات هذا المريض تحديدًا).
// هذا الفصل ضروري حتى تنفع ميزة Prompt Caching بجهة الباك اند (Code.gs):
// الكاش يخزّن بادئة (prefix) ثابتة، فإذا الجزء الثابت (المنهجية + شكل الـ
// JSON المطلوب + كل القواعد) محشور مع بيانات مريض متغيّرة بنفس النص، ما
// ينفع نخزّنه بكاش لأنه يتغيّر كل مرة. بعد الفصل، الجزء الثابت (~2500+
// توكن) يُخزَّن مرة وحدة ويُعاد استخدامه بسعر أرخص بكثير بكل تحليل جاي
// خلال نافذة الكاش (٥ دقايق افتراضيًا)، بدل ما يُعاد حسابه بالسعر الكامل
// من الصفر كل مرة
async function analyzeRecord(record, answers) {
  ...
  "missing_information": ["short description of any information that was genuinely missing/blank and limited this analysis"]
        ...
        "IMPORTANT: patient-facing text values (patient_summary.arabic, personalized_instructions, oral_hygiene_aids, smart_goals, barriers_and_solutions.solution_arabic, motivational_message_arabic) must always be written in ARABIC (simple, clear Arabic the patient can understand) regardless of the current interface language, since the patient reads Arabic. Keep dentist_summary in English as specified.",
        AI_SAFETY
    ].join("\n");

    // الجزء المتغيّر فقط: بيانات هذا المريض تحديدًا — هذا وحده يُعاد حسابه
    // بالسعر الكامل كل مرة (صغير نسبيًا مقارنة بالجزء الثابت أعلاه)
    const prompt = [
    ...
    // 6000 بدل 4600: الرد يشمل video_evaluation لكل فيديو بالقائمة (مو بس
    // المُرشّح منها)، فكلما زاد عدد الفيديوهات المتاحة كبر الرد — رفع الحد
    // الأساسي هذا يقلل احتمال القطع بأول محاولة، خصوصًا بعد ما صرنا لا نعيد
    // محاولة الشبكة تلقائيًا (انظر callAIJSON)
    const result = await callAIJSON(prompt, { system, maxTokens: 6000, temperature: 0.4 });
...
async function runAIAnalysis() {
    ...
    // 2) ثم التحليل — نطلب إبقاء الشاشة صاحية طول مدة التحليل، ونحررها
    // دائمًا (نجاح أو فشل) حتى ما تبقى الشاشة صاحية بلا داعٍ بعدين
