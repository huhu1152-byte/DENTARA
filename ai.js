...
const DENTARA_METHODOLOGY = [
    ...
    "",
    "STRICT VIDEO RULE — choose videos ONLY from the numbered list given below, by index. Never invent a title, id, or url. Choose only the videos that truly match this patient's top-3 priority problems, ranked by relevance — if genuinely none of the listed videos fit this patient, return an empty videoIndexes array. Do not pick a video just because it exists or to avoid an empty list; an irrelevant video is worse than no video.",
    "",
...
async function analyzeRecord(record, answers) {
  ...
  "videoReason": "short reason for choosing these videos"
        ...
        "",
        "Write 4 to 5 practical instructions the patient can follow at home, ordered by importance, derived from the top-3 priority problems (not a generic list).",
