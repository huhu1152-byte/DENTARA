...
function viewAssessment(id) {
    ...
    let priorityProblems = [], personalizedInstructions = [], oralHygieneAids = [], smartGoals = [],
        barriersSolutions = [], recommendedVideos = [], aiExplainability = [], followUp = {},
        missingInformation = [], answers = {};
    try { priorityProblems = JSON.parse(a.priorityProblems || "[]"); } catch (e) {}
    try { personalizedInstructions = JSON.parse(a.personalizedInstructions || "[]"); } catch (e) {}
    try { oralHygieneAids = JSON.parse(a.oralHygieneAids || "[]"); } catch (e) {}
    try { smartGoals = JSON.parse(a.smartGoals || "[]"); } catch (e) {}
    try { barriersSolutions = JSON.parse(a.barriersSolutions || "[]"); } catch (e) {}
    try { recommendedVideos = JSON.parse(a.recommendedVideos || "[]"); } catch (e) {}
    try { aiExplainability = JSON.parse(a.aiExplainability || "[]"); } catch (e) {}
    try { followUp = JSON.parse(a.followUp || "{}"); } catch (e) {}
    try { missingInformation = JSON.parse(a.missingInformation || "[]"); } catch (e) {}
    ...
    App.current = {
        ...
        record: a,
        result: {
            patient_summary: { arabic: a.patientSummaryAr || "", dentist_summary: a.dentistSummary || "" },
            risk_assessment: { level: a.riskLevel || "", supporting_factors: String(a.riskFactors || "").split("|").map(s => s.trim()).filter(Boolean) },
            priority_problems: priorityProblems,
            personalized_instructions: personalizedInstructions,
            oral_hygiene_aids: oralHygieneAids,
            smart_goals: smartGoals,
            barriers_and_solutions: barriersSolutions,
            recommended_videos: recommendedVideos,
            ai_explainability: aiExplainability,
            motivational_message_arabic: a.motivationalMessage || "",
            follow_up: followUp,
            missing_information: missingInformation,
            dentist_review_required: String(a.dentistReviewRequired || "").toLowerCase() !== "no"
        }
    };
