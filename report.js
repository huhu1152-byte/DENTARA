...
// نعرض شاشة النتائج بلغة التحليل المحفوظة مع السجل (instrLang) لا لغة
// الواجهة الحالية — نفس منطق الدفتر (bookletLang) وبنفس السبب: قد يبدّل
// الطبيب لغة الواجهة بعد التحليل، فتختلط تسميات ثابتة إنكليزية مع محتوى
// عربي مولَّد أو العكس. الاستبدال مؤقت فقط أثناء بناء هذه الشاشة.
function withRecordLang(fn, recOverride) {
    const rec = recOverride || (App.current && App.current.record);
    const savedLang = LANG;
    if (rec && (rec.instrLang === "ar" || rec.instrLang === "en")) LANG = rec.instrLang;
    try { return fn(); } finally { LANG = savedLang; }
}

// ======== شاشة النتائج داخل التطبيق ========
function renderResults(r) { withRecordLang(() => _renderResults(r)); }

function _renderResults(r) {
    const p = App.current.patient;
    const videos = pickedVideos(r);
    const risk = r.risk_assessment || {};
    const level = risk.level || "غير محدد";
    const cls = riskClass(level);
    const problems = r.priority_problems || [];
    const instructions = r.personalized_instructions || [];
    const aids = r.oral_hygiene_aids || [];
    const goals = r.smart_goals || [];
    const barriers = r.barriers_and_solutions || [];
    ...
    $("resultsBody").innerHTML = `
        ...
        ${(r.patient_summary || {}).arabic ? `
        <div class="res-section">
            <p>${escapeHtml(r.patient_summary.arabic)}</p>
        </div>` : ""}

        <div class="risk-card ${cls}">
            <span>${t("riskLevel")}</span>
            <b>${escapeHtml(level)}</b>
            ${(risk.supporting_factors || []).length ? `<p>${escapeHtml(risk.supporting_factors.join("، "))}</p>` : ""}
        </div>

        ${(r.ai_explainability || []).length ? `
        ...
        <div class="res-section">
            ...
            ${problems.map((pr, i) => `
                <div class="prob-row"><span class="prob-num">${pr.priority || (i + 1)}</span>${escapeHtml(pr.problem || "")}</div>
            `).join("")}
        </div>` : ""}
        ...
        <div class="res-section">
            ...
            ${instructions.map(ins => `
                <div class="ins-row">
                    <b>${escapeHtml(ins.instruction_arabic || "")}</b>
                    <p>${escapeHtml(ins.how_to_do_it_arabic || "")}</p>
                    ${ins.habit_tip_arabic ? `<p class="ins-tip"><i class="fa-solid fa-lightbulb"></i> ${escapeHtml(ins.habit_tip_arabic)}</p>` : ""}
                    ${ins.when ? `<span class="ins-times"><i class="fa-solid fa-clock"></i> ${escapeHtml(ins.when)}</span>` : ""}
                </div>`).join("")}
        </div>

        ${aids.length ? `
        <div class="res-section">
            <h3><i class="fa-solid fa-toolbox"></i> ${t("oralHygieneAids")}</h3>
            ${aids.map(a => `
                <div class="ins-row">
                    <b>${escapeHtml(a.aid || "")}</b>
                    <p>${escapeHtml(a.how_to_use_arabic || "")}</p>
                    ${a.frequency ? `<span class="ins-times"><i class="fa-solid fa-repeat"></i> ${escapeHtml(a.frequency)}</span>` : ""}
                    ${a.precautions ? `<p class="ins-tip"><i class="fa-solid fa-circle-exclamation"></i> ${escapeHtml(a.precautions)}</p>` : ""}
                </div>`).join("")}
        </div>` : ""}
        ...
        <div class="res-section">
            ...
            <ul class="goal-list">${goals.map(g => `<li>${escapeHtml(g.goal_arabic || "")}${g.time_frame ? ` — <small>${escapeHtml(g.time_frame)}</small>` : ""}</li>`).join("")}</ul>
        </div>` : ""}
        ...
        <div class="res-section">
            ...
            ${barriers.map(b => `
                <div class="barrier-row">
                    ...
                    <p><i class="fa-solid fa-circle-check"></i> ${escapeHtml(b.solution_arabic || "")}</p>
                </div>`).join("")}
        </div>` : ""}

        ${r.motivational_message_arabic ? `
        <div class="motivation-card">
            <i class="fa-solid fa-heart"></i>
            <p>${escapeHtml(r.motivational_message_arabic)}</p>
        </div>` : ""}
            ...
            ${videos.map(v => `
                <div class="video-row">
                    ...
                    <div>
                        ...
                        ${v._reason ? `<span>${escapeHtml(v._reason)}</span>` : (v.description ? `<span>${escapeHtml(v.description)}</span>` : "")}
                    </div>
                </div>`).join("")}
        </div>` : `
        <div class="res-section">
        ...
        ${missing.length ? `
        <div class="res-section res-missing-info">
            <h3><i class="fa-solid fa-circle-info"></i> ${t("missingInformation")}</h3>
            <ul class="goal-list">${missing.map(m => `<li>${escapeHtml(m)}</li>`).join("")}</ul>
        </div>` : ""}

        ${followUp.recommended ? `
        <div class="res-section res-followup-info">
            <h3><i class="fa-regular fa-calendar"></i> ${t("followUp")}</h3>
            <p>${escapeHtml(followUp.time_frame || "")}${followUp.reason ? " — " + escapeHtml(followUp.reason) : ""}</p>
        </div>` : ""}
    `;
...
function openAiReasoning() { withRecordLang(_openAiReasoning); }

function _openAiReasoning() {
    const r = App.current.result;
    if (!r || !(r.ai_explainability || []).length) return;

    openModal(t("whyAITitle"), `
        <p class="modal-note">${t("whyAIIntro")}</p>
        ${r.ai_explainability.map((e, i) => `
            <div class="bk-how">
                <span class="bk-how-num">${i + 1}</span>
                <p><b>${escapeHtml(e.recommendation || "")}</b><br>${escapeHtml(e.reason || e.finding || "")}</p>
            </div>`).join("")}
    `, null);
}
...
function showResultsUnanalyzed(record, err) { withRecordLang(() => _showResultsUnanalyzed(record, err)); }

function _showResultsUnanalyzed(record, err) {
...
// الفيديوهات التي اختارها النموذج (بمعرّف id الحقيقي، مرتبة حسب priority)
function pickedVideos(r) {
    return (r.recommended_videos || [])
        .slice()
        .sort((a, b) => (a.priority || 0) - (b.priority || 0))
        .map(rv => {
            const v = App.videos.find(x => String(x.id) === String(rv.video_id));
            return v ? { ...v, _reason: rv.reason || "", _relatedProblem: rv.related_problem || "" } : null;
        })
        .filter(Boolean);
}
...
function renderAssessmentComplete() {
    ...
    list.forEach(a => {
        ...
        try {
            (JSON.parse(a.priorityProblems || "[]")).forEach(pr => bump(problemsMap, pr.problem));
        } catch (e) {}
    });
...
<script>window.onload = function () { setTimeout(function () { window.print(); }, 300); };<\/script>
    ...
    if (isIOS) {
        // ملاحظة مهمة: window.open(blobURL, "_blank") يفتح غالبًا تبويبًا
        // أبيض فارغًا على iOS Safari تحديدًا (قصور معروف في WebKit مع
        // Blob URLs عبر window.open) — حتى لو نجح على متصفحات أخرى. الحل
        // الموثوق: نفتح نافذة فارغة أولاً (ضمن استجابة النقرة مباشرة حتى
        // لا يُحظر كنافذة منبثقة)، ثم نكتب المحتوى داخلها مباشرة عبر
        // document.write بدل الاعتماد على Blob URL إطلاقًا
        const win = window.open("", "_blank");
        if (!win) {
            // إن حُظرت النافذة المنبثقة، نطبع من الصفحة الحالية كحل أخير
            window.print();
            return;
        }
        win.document.open();
