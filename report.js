...
function _openAiReasoning() {
    ...
    const problems = (r && r.priority_problems) || [];
    ...
    openModal(t("whyAITitle"), `
        ...
        ${problems.map((pr, i) => `
            <div class="bk-how">
                ...
                <p><b>${escapeHtml(pr.problem || "")}</b><br>${escapeHtml(pr.why_prioritized || "")}${
                    (pr.evidence || []).length ? `<br><i>${escapeHtml(pr.evidence.join("، "))}</i>` : ""}</p>
            </div>`).join("")}
    `, null);
}
